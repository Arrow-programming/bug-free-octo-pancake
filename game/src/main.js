...
import { spritesheet, gfx } from "../assets/art/pixelart.js";
    ...
    */
    ...
    const WATER_TILT_PER_ROW = -Math.tan(WATER_LIGHT_ANGLE_DEG * Math.PI / 180) * (blockSize / 9);
    ...
    class LightStreak {
        ...
        centerAt(depth, t, surfaceDepth = null) {
            ...
            const anchorFactor = surfaceDepth === null ? 1 : constrain(surfaceDepth / this.anchorDepth, 0, 1);
            return this.x
                + depth * this.tilt
                + sway * anchorFactor;
        }
    ...
    function drawWaterTile(x, y, w, h, topSurface) {
        ...
        for (let row = minRow; row < rows; row++) {
            ...
            for (let col = 0; col < cols; col++) {
                ...
                if (wide) {
                    ctx.globalAlpha = constrain((wide.opacity + disturbance.glow * 0.6) * depthFade, 0, 1);
                ...
                if (fine) {
                    ctx.globalAlpha = constrain((fine.opacity + disturbance.glow * 0.4) * depthFade, 0, 1);
    ...
    class WaterSurfaceSegment {
        ...
        indexFor(worldX) {
            ...
            return constrain(Math.round(rel), 0, this.count - 1);
        }
        ...
        heightAt(worldX) {
            ...
            const rel = constrain((worldX - this.startX) / this.spacing, 0, this.count - 1);
        ...
        update(dt) {
            ...
            for (let i = 0; i < this.count; i++) {
                this.heights[i] = constrain(this.heights[i], -WATER_SPRING.maxDisplacement, WATER_SPRING.maxDisplacement);
            }
    ...
    function buildWaterSurfaceSegments() {
        ...
        for (const colX of columns) {
            ...
            if (segStart === null) {
                segStart = colX;
                segY = colY;
            } else if (colX !== prevX + blockSize || colY !== segY) {
                waterSurfaceSegments.push(new WaterSurfaceSegment(segStart, prevX + blockSize, segY));
                segStart = colX;
                segY = colY;
            }
            prevX = colX;
    ...
    function waterDisplacementAt(worldX) {
        ...
        return constrain(total, -WATER_SPRING.maxDisplacement, WATER_SPRING.maxDisplacement);
    }

    function waterSurfaceLineAt(worldX) {
        const blockX = Math.floor(worldX / blockSize) * blockSize;
        const restY = waterSurfaceByColumn[blockX];
        if (restY === undefined) {
            return null;
        }
        const cellH = blockSize / 9;
    ...
    function backgroundRippleBendAt(worldX, worldY) {
        ...
        return constrain(bend, -1, 1) * BG_RIPPLE.amp;
    }
    ...
    function updatePlayerWaterSpring(dt) {
        ...
        if (player.inWater && !player.wasInWater) {
            ...
            spawnWaterLightDisturbance(centerX, constrain(impactSpeed * 0.5, 10, 60));
            if (player.v.y > 0) {
            ...
            spawnWaterLightDisturbance(centerX, constrain(Math.abs(exitSpeed) * 0.35, 12, 60));
        }
        if (player.inWater) {
            ...
            if (depthUnderRest > -WATER_SPRING.pressBand && depthUnderRest < WATER_SPRING.pressBand) {
                const target = constrain(depthUnderRest * 0.18, -WATER_SPRING.maxDisplacement, WATER_SPRING.maxDisplacement);
    ...
    function applyUnderwaterDistortion() {
        ...
        for (let y = 0; y < height; y += rowH) {
            ...
            //quantize the shift to whole in-game pixels (multiples of PIXEL) so the warp
            //looks like the pixel art is stepping, not sliding around at sub-pixel amounts
            const shift = Math.round((wave * UNDERWATER_DISTORT.amp) / PIXEL) * PIXEL;
            ctx.drawImage(
                frameBuffer,
    ...
    */
    ...
    function ensureBgFrameBuffer(pad) {
        const neededW = width + pad * 2;
        if (!bgFrameBuffer || bgFrameBuffer.width !== neededW || bgFrameBuffer.height !== height) {
            bgFrameBuffer = document.createElement("canvas");
            bgFrameBuffer.width = neededW;
            bgFrameBuffer.height = height;
    ...
    function drawBackgroundArt() {
        ...
        bgFrameBufferCtx.fillRect(0, 0, bgFrameBuffer.width, height);
        ...
        if (backdropSprite) {
            ...
            const spriteH = backdropSprite.h * pixel;
            const offsetX = ((Math.floor(-cam.x * 0.08) % spriteW) + spriteW) % spriteW;
            for (let x = -pad - spriteW; x < width + pad + spriteW; x += spriteW) {
                for (let y = -spriteH; y < height + spriteH; y += spriteH) {
        ...
        for (let y = 0; y < height; y += rowH) {
            const rh = Math.min(rowH, height - y);
            const worldY = y + cam.y - shake.y - height / 2;
            ...
            const shift = constrain(Math.round(bend / PIXEL) * PIXEL, -pad, pad);
            ctx.drawImage(
                bgFrameBuffer,
                pad - shift, y, width, rh,
                0, y, width, rh
    ...
    const player = {
        ...
        moveX: function(){
            ...
            if (this.inWater) {
                ...
                this.v.x = constrain(this.v.x, -this.speed * speedMul, this.speed * speedMul);
                this.x += this.v.x * dt;
                this.action = "air";
                return;
            }
        ...
        moveY: function(){
            ...
            if (this.inWater) {
                ...
                this.v.y *= 0.988;
                this.v.y = constrain(this.v.y, -WATER_SPRING.maxSwimSpeed, WATER_SPRING.maxSwimSpeed);
                this.y += this.v.y * dt;
                this.action = "air";
                return;
            }
        ...
        display: function() {
            ...
            const frameDuration = stateKey === "run"
                ? constrain(0.16 - Math.abs(this.v.x) / this.speed * 0.1, 0.045, 0.16)
                : 0.12;
            ...
            const spriteW = frame.w * PIXEL;
            const spriteH = frame.h * PIXEL;

            const rawX = this.x + this.w / 2 - spriteW / 2 - 3;
            const rawY = this.y + this.h - spriteH + 10;
            const drawX = Math.round(rawX / PIXEL) * PIXEL;
            const drawY = Math.round(rawY / PIXEL) * PIXEL;

            frame.draw(ctx, drawX, drawY, PIXEL, this.dir);
        }
    };
    ...
    function blockAt(worldX, worldY) {
        const col = Math.floor(worldX / blockSize);
        const row = Math.floor(worldY / blockSize);
    ...
    class RainSplashParticle {
        ...
        draw() {
            const t = constrain(1 - this.life / this.maxLife, 0, 1);
            ctx.globalAlpha = t * 0.85;
            ctx.fillStyle = this.color;
            const px = Math.round(this.x / PIXEL) * PIXEL;
            const py = Math.round(this.y / PIXEL) * PIXEL;
    ...
    function spawnImpactSplash(x, y) {
        ...
        for (let i = 0; i < count; i++) {
            const offsetX = Math.round((Math.random() - 0.5) * 3) * PIXEL;
            const offsetY = Math.round((Math.random() - 0.5) * 2) * PIXEL;
    ...
    class RainDrop {
                ...
                if (hit) {
                    if (hit.type === "water") {
                        ...
                        if (!isSurface || ny >= surfaceLine) {
                            this.alive = false;
                            const impact = constrain(Math.abs(this.vy) * 0.12, 6, 42);
                            splashWaterSurface(nx, impact);
                            if (Math.random() < 0.45) {
                                spawnWaterLightDisturbance(nx, constrain(impact * 0.5, 6, 24));
                            }
        ...
        draw() {
            ...
            for (let s = 0; s < this.len; s++) {
                const px = Math.round((baseX - ux * s * size) / PIXEL) * PIXEL;
                const py = Math.round((baseY - uy * s * size) / PIXEL) * PIXEL;
    ...
    function updateRainLayers(dt) {
        ...
        for (const key in RAIN.layers) {
            ...
            const spawnY = centerY - height / 2 - spawnMarginY;
            const slant = Math.abs(Math.tan(angleRad)) * (height + spawnMarginY);
            const spawnLeft = centerX - width / 2 - slant - 80;
            const spawnRight = centerX + width / 2 + slant + 80;
    ...
    class Fire{
        ...
        constructor(x,y,w,h){
            ...
            this.h = h;
            this.t = 0;
            this.img = ctx.createImageData(~~(this.w/Fire.gridSize),~~(this.h/Fire.gridSize));
    ...
    ctx.drawImage(await toBitmap(texStr.img),0,0);
    let texture = ctx.getImageData(0,0,600,600);
    ...
    const rasterizer = new Rasterizer(3);
    ...
    class Lighting {
        ...
        getLightLevel(x, y, symbol) {
            ...
            const smoothed = lerp(previous, target, this.smoothing);
    ...
    function mapSetup(LEVEL) {
            ...
            for (let j = 0; j < MAP[LEVEL][i].length; j++) {
                ...
                switch (sym) {
					...
					case "F": {
                        ...
                        blocks.push(new FireBlock(j * blockSize, i * blockSize, blockSize * (runEnd - j + 1), blockSize));
                        j = runEnd;
                        break;
                    }
                    case "@":
        ...
        blockGrid = new Map();
        for (const block of blocks) {
            const colStart = Math.round(block.x / blockSize);
            const colEnd = Math.round((block.x + block.w) / blockSize) - 1;
            const row = Math.round(block.y / blockSize);
    ...
    const render = (timestamp) => {
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (block.type !== "water" || !edgeCheck(player, block)) continue;
            if (block.isTopSurface && block.isTopSurface()) {
